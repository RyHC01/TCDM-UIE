...
from core.data import create_dataloader, create_dataset
...
def init_loggers(opt):
    ...
    logger = get_root_logger(
        logger_name='core', log_level=logging.INFO, log_file=log_file)
...
def main():
    ...
    state_folder_path = 'uie_exps/{}/training_states/'.format(opt['name'])
    import os
    try:
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            gt = train_data['gt']
            if raw.ndim == 5:
                raw = raw.flatten(start_dim=0, end_dim=1)
            if gt.ndim == 5:
                gt = gt.flatten(start_dim=0, end_dim=1)

            if raw.size(0) > batch_size:
                indices = random.sample(range(0, raw.size(0)), k=batch_size)
                raw = raw[indices]
                gt = gt[indices]
            ...
            if gt_size is not None:
                ...
                if h > gt_size or w > gt_size:
                    x0 = int((h - gt_size) * random.random())
                    y0 = int((w - gt_size) * random.random())
                    x1 = x0 + gt_size
                    y1 = y0 + gt_size
                    raw = raw[:, :, x0:x1, y0:y1]
                    gt = gt[:, :, x0 * scale:x1 * scale, y0 * scale:y1 * scale]

            model.feed_train_data({'raw': raw, 'gt': gt})
