...
from core.models.archs import define_network
from core.models.base_model import BaseModel
from core.utils import get_root_logger, imwrite, tensor2img

loss_module = importlib.import_module('core.models.losses')
metric_module = importlib.import_module('core.metrics')
...
class UIEModel(BaseModel):
    ...
    def feed_train_data(self, data):
        self.raw = data['raw'].to(self.device)
        if 'gt' in data:
            self.gt = data['gt'].to(self.device)

        if self.mixing_flag:
            self.gt, self.raw = self.mixing_augmentation(self.gt, self.raw)
    ...
    #     self.raw = self.data_transform(self.raw)
    #     self.gt = self.data_transform(self.gt)
    #
    #     n = self.raw.size(0)
    #     e = torch.randn_like(self.raw)
    #     b = self.betas
    ...
    #
    #     preds = self.net_g(torch.cat([self.raw, x], dim=1), t.float())
    #
    ...
    def optimize_parameters(self, current_iter):
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            t = (torch.ones(n) * i).to(self.raw.device)
            next_t = (torch.ones(n) * j).to(self.raw.device)
            at = self.compute_alpha(self.betas, t.long())
            at_next = self.compute_alpha(self.betas, next_t.long())
            xt = xs[-1]
            et = self.net_g(torch.cat([self.raw, xt], dim=1), t)
        ...
        loss_dict = OrderedDict()
        ssim_loss = (1 - self.cri_pix1(xt_next, self.gt))
        l1_loss = self.cri_pix2(xt_next, self.gt)
        perceptual_loss = self.cri_pix3(xt_next, self.gt)

        l_pix = 0.84 * ssim_loss + l1_loss + 0.02 * perceptual_loss

        loss_dict['l_pix'] = l_pix
        loss_dict['ssim_loss'] = 0.84 * ssim_loss
        loss_dict['l1_loss'] = l1_loss
        loss_dict['perceptual_loss'] = 0.02 * perceptual_loss
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            img_name = osp.splitext(osp.basename(val_data['raw_path'][0]))[0]

            self.feed_data(val_data)
            self.raw = self.raw.flatten(start_dim=0, end_dim=1) if self.raw.ndim == 5 else self.raw
    ...
    def get_current_visuals(self):
        ...
        out_dict['raw'] = self.raw.detach().cpu()
